...
-- 6 название альбомов, в которых присутствуют исполнители более 1 жанра - NO
...
join genser on genser_perfomers.genser_id = genser.id
GROUP by albums.title
HAVING count(albums.title) > 2;


-- 7 наименование треков, которые не входят в сборники - NO
select id, title from traks
join collectin_trak on traks.id = collectin_trak.trak_id
where not id;

-- 8 исполнителя(-ей), написавшего самый короткий по продолжительности трек - NO
select performers.title from performers
